...
@ Constants
        .equ    STDOUT,1
        .equ    alpha,-20     @ for offset in register
...
main:
        ...
        mov   r4, 'A          @ move numeral 0 to r4

@ loop
loop:
        strb  r4, [fp, alpha]   @ store register byte in r4
                                @ using numeral offset
        mov   r0, STDOUT        @ write to stdout
        add   r1, fp, alpha   @ addr of numeral in r1
        mov   r2, 1             @ 1 byte
        bl    write

        add   r4, r4, 1         @ next numeral
        cmp   r4, 'Z            @ have gone past max numeral?
